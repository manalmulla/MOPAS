...
// ── Data ──────────────────────────────────────────────────────────────────
const THREATS = [
  { type:"URL",   target:"paypal-security[.]net",      sev:"CRITICAL", time:"2m ago" },
  { type:"EMAIL", target:"invoice@supp1ier.com",       sev:"HIGH",     time:"7m ago" },
  { type:"QR",    target:"qr redirect → malware[.]io", sev:"HIGH",     time:"15m ago" },
  { type:"IMG",   target:"logo_spoof_apple.png",       sev:"MEDIUM",   time:"31m ago" },
  { type:"URL",   target:"amazon-login-verify[.]tk",   sev:"CRITICAL", time:"1h ago" },
  { type:"EMAIL", target:"hr@cornpany.com",            sev:"LOW",      time:"2h ago" },
];
...
// ── Dashboard data ────────────────────────────────────────────────────────
const STATS = [
  { label:"Threats Blocked", value:"2,847",  delta:"+12 today", color:"var(--success)" },
  { label:"URLs Scanned",    value:"14,203", delta:"+89 today", color:"var(--accent)" },
  { label:"Phishing Caught", value:"342",    delta:"-5 today",  color:"var(--danger)" },
  { label:"System Score",    value:"98.2%",  delta:"+0.3 today",color:"var(--warn)" },
];
const BARS = [
...
const SYSTEMS = [
  ...
  { label:"ML Model v4.2",  status:"SYNCING", color:"var(--warn)" },
    ...
    <div className="fade-in-up" style={{ animationDelay:"0.28s", background:"var(--card)", border:"1px solid var(--border)", borderRadius:10, overflow:"hidden" }}>
      ...
      {THREATS.map((t,i) => (
        <div key={i} className="threat-row">
          <span className="badge" style={{ color:SEV[t.sev], borderColor:`${SEV[t.sev]}40` }}>{t.type}</span>
          <span className="font-mono" style={{ flex:1, fontSize:12, color:"var(--text)" }}>{t.target}</span>
          <span className="font-display" style={{ fontSize:10, fontWeight:700, letterSpacing:2, color:SEV[t.sev] }}>{t.sev}</span>
          <span className="font-mono" style={{ fontSize:10, color:"var(--muted)", minWidth:48, textAlign:"right" }}>{t.time}</span>
        </div>
      ))}
    </div>
